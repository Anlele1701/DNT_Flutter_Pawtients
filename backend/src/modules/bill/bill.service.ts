...
export class BillService {
  ...
  async createBill(
    ...
    createBillDto: CreateBillDto | string,
  ): Promise<Bill> {
    try {
      ...
      file.forEach((item) => {
        createBillObj.hinhAnh.push(
          new Image(item.originalname, item.buffer, item.mimetype),
        );
      });
      const newBill = new this.billModel(createBillObj);
      await newBill.save();
      if (newBill) {
        var appointment = await this.appointmentModel.findOne({
          _id: newBill.idLichKham,
        });
  ...
  async getBill(id: string): Promise<Bill> {
    try {
      const bill = await this.billModel.findOne({ _id: id });
      if (bill) return bill;
      else return null;
    } catch (e) {
      console.log(e);
